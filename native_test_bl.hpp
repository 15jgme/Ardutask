...
{
    float currentRate = 1000.0f/float(GetTickCount()-tlast);
    rateSum += double(currentRate);
    runningAvgRate = float(rateSum) / float(iters);
    tr = (runningAvgRate < (1.0f + TEST_TOLERANCE_PERCENT)*getRate()) && (runningAvgRate > (1.0f - TEST_TOLERANCE_PERCENT)*getRate());
    tlast = GetTickCount();
    iters++;
    return 1;
}